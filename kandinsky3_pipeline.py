...
from diffusers.models import Kandinsky3UNet, VQModel
...
def numpy_to_bd(images):
    ...
    if images.shape[-1] == 1:
        ...
        for image in images:
            buf = BytesIO()
            Image.fromarray(image.squeeze(), mode = "L").save(buf, format = "PNG")
            pil_images.append(buf.getvalue())
    else:
        for image in images:
            buf = BytesIO()
